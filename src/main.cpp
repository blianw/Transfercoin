...
uint256 hashGenesisBlock("0x411a652a28e165d2cbeb18496f0dfe9e05adc393ab92fc409c776d93978a6880");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // transfercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "transfercoin Signed Message:\n";
...
{
    // transfercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight < 10080)
    {  nSubsidy = 50 * COIN;}
    else if (nHeight < 31680)
    { nSubsidy =25 * COIN;}
    else if (nHeight < 74880)
    { nSubsidy = 15 * COIN;}
    else if (nHeight < 139680)
    { nSubsidy = 10 * COIN;}
    else if (nHeight < 182880)
    {nSubsidy = 7 * COIN;}
    else if (nHeight < 3550560)
    {nSubsidy =5 * COIN;}
    else
    {nSubsidy =0;}
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
   // nSubsidy >>= (nHeight / 840000); // transfercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60; // transfercoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // transfercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // transfercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // transfercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // transfercoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x411a652a28e165d2cbeb18496f0dfe9e05adc393ab92fc409c776d93978a6880");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today,The Transfer coins is building to world for now.....";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388365364;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086080625;
        ...
        assert(block.hashMerkleRoot == uint256("0xbd4de436b855209383e404e31490ea3ce1640599c92e0afbdab4690700dc416b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // transfercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("transfercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("transfercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static transfercoinMiner(CWallet *pwallet)
{
    printf("transfercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("transfercoin-miner");
    ...
    try { loop {
        ...
        printf("Running transfercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("transfercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&transfercoinMiner, pwallet));
}
