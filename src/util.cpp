...
#else
    const char* pszModule = "transfercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "transfercoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "transfercoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "transfercoind.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // transfercoin: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
